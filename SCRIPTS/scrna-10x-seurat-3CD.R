...
integrate_seurat_obj = function(original_wd, sample_analysis_dirs, num_dim) {
  ...
  # dont forget this C.D.!#
  anchors = FindIntegrationAnchors(object.list = seurat_obj_list, anchor.features = 5000, dims = 1:num_dim, reference = 1)
  rm(seurat_obj_list)
...
calculate_variance = function(seurat_obj, jackstraw_max_cells = 10000) {
  ...
  s_obj = FindVariableFeatures(s_obj, selection.method = "vst", nfeatures = 4000, verbose = FALSE)
...
calculate_variance_integrated = function(seurat_obj, num_dim) {
  ...
  ## COOPER RMEMEBER TO UNDO CHANGES TO RUNPCA AND CALC INT VAR
  ...
  s_obj = RunTSNE(s_obj, reduction = "pca", dims.use = 1:num_dim, check_duplicates=FALSE)
...
calculate_cluster_stats = function(seurat_obj, label) {
  ...
  tsne_tbl = seurat_obj[["umap"]]@cell.embeddings %>% round(3) %>% as.data.frame() %>% rownames_to_column("cell")
...
calculate_cluster_de_genes = function(seurat_obj, label, test, group_var = "orig.ident") {
  ...
  for (clust_name in clusters) {
    ...
    Idents(clust_obj) = clust_obj@meta.data$pouch.status
...
if (opts$create) {
  ...
  seurat_obj@assays$integrated@data[is.na(seurat_obj@assays$integrated@data)]<-0
