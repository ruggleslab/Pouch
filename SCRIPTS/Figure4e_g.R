...
meta = data.frame(ileum_ldm$dataset$cell_to_cluster)
meta$sampleID = ileum_ldm$dataset$cell_to_sample
states = ileum_ldm$clustAnnots
colnames(meta)[1] <- "clusterNum"

meta$clusterName=NA
meta$clusterName=NA
for ( i in 1:length(states)){
  meta$clusterName[meta$clusterNum == names(states)[i]] <- states[i]
}

#patient ids
addt = read.table("additional_data/Martin_et_al/GIMATS_meta.txt", T, '\t')
addt = subset(addt, tissue=="ILEUM")

samps = unique(meta$sampleID)
rownames(addt) = addt$Sample_ID

meta$PatientID = NA
meta$Inflammation = NA
for(i in 1:length(samps)){
  meta$PatientID[meta$sampleID==samps[i]]<-as.character(addt[samps[i],]$Patient.ID)
  meta$Inflammation[meta$sampleID==samps[i]]<-as.character(addt[samps[i],]$status)
}

meta$Barcode=rownames(meta)
write.table(meta, "additional_data/Martin_et_al/GIMAT_full_meta.txt", quote=F, sep='\t', row.names=F)

####
#counts
library(Matrix)
counts = ileum_ldm$dataset$umitab

sig = read.table("OBJECTS/Signature/signatureRefCt.txt", T, '\t')
overlapper = intersect(sig$gene, rownames(counts))
counts2 = as.matrix(counts[overlapper,])

counts_agg <- aggregate(t(counts2), list(meta$sampleID, meta$clusterName, meta$Inflammation, meta$PatientID), mean)
count_summ = t(counts_agg[,5:ncol(counts_agg)])
colnames(count_summ) = paste0(counts_agg$Group.2, "_", counts_agg$Group.3, "_", counts_agg$Group.1, "_", counts_agg$Group.4)
meta_sum = counts_agg[,1:4]
colnames(meta_sum) <- c("SampleID", "cellState", "Inflammation", "PatientID")


##subset immune cell types
meta_imm = subset(meta_sum, grepl("CD8|T cell|DC|plasma|ILC|Macro|macro|mast|B cell|gd|TFH|Th17|Treg|Trm", meta_sum$cellState, ignore.case = T))
meta_imm = meta_imm[order(meta_imm$cellState),]

count_imm = count_summ[,as.numeric(rownames(meta_imm))]
rownames(meta_imm) <- colnames(count_imm)
...
count_simp = aggregate(t(count_imm), by=list(meta_imm$cellState), mean)
...
their_t = c("CD8 Trm","Central Memory T cells", "Cytotoxic CD8 Trm","Cytotoxic T cells", "Highly activated T cells",
            "ILC1", "ILC3", "Naive T cells", "T (gd)","TFH-like","Th17 Trm","Tregs","Trm")
...
their_b = c("IgA plasma cells","IgG plasma cells", "IgM plasma cells", "Memory B cells", "Naive B cells", "Plasmablasts")

our_m = c("mono_mac1", "mono_mac2", "mono_mac3", "mast_cells", "pdcs")

their_m =c("Mast cells", "DC1", "DC2", "Inf. Macrophages", "mature DC", "moDC", "pDC", "Resident macrophages")
...
namer <- "FIGURES/FIGURE4/Figure4e_martin_heat.pdf"
pdf(namer, height = 8, width = 20)
grid.arrange(heatty_t, heatty_m, heatty_b, nrow=1, widths = c(1.6,1.2,1.3))
dev.off()
...
#
### clean up metadata
ram_totals = data.frame(table(meta$sampleID))
colnames(ram_totals) = c("Sample", "cellTotal")
samps = as.character(unique(ram_totals$Sample))
health = c()
for(i in 1:length(samps)){
  curr = subset(meta, sampleID == samps[i])
  new_h = as.character(curr$Inflammation[1])
...
for( i in 1:length(samps)){
  ...
  meta$cellTotals[meta$sampleID==samps[i]] <- CT
}
...
colorer = c("Involved"="red3", "Uninvolved"="purple3", "other"="grey85")
...
mac_val = data.frame(cells = colnames(counts3),
                     ...
                     IL1B=il1B,LYZ =lyz,
                     APOE = apoe,C1QC=c1qc,
                     Health = meta$Inflammation,
                     Cluster = meta$clusterName,
                     cellTotal = meta$cellTotals,
                     sample = meta$sampleID)

mac_val = subset(mac_val, !is.na(Health))

mac_val$Health = factor(mac_val$Health, levels = c("Involved", "Uninvolved", "other"))

##mono_mac1
plotter = subset(mac_val, SOX4 > 0 & MAFB > 0)
plotter = subset(mac_val, grepl("Monocyte|Macropha", mac_val$Cluster, ignore.case = T))
...
resser$Inflammation = factor(resser$Inflammation, levels = c("Involved", "Uninvolved", "other"))
...
plotter$Inflammation2 <- plotter$Health
plotter$Inflammation2[plotter$IL1B < 0] <- "other"
plotter$Inflammation2[plotter$LYZ < 0] <- "other"
...
plotter_int <- subset(plotter, IL1B > 0 & LYZ > 0)
samples <- unique(plotter_int$sample)
resser <- data.frame(sampleID = NA, Inflammation = NA, perc = NA)
for(j in 1:length(samples)){
  curr <- subset(plotter_int, sample == samples[j])
...
plotter = subset(mac_val, grepl("CD8|CD4|Treg|TFH|Cycling T|NKs|Naive T|gd|Trm", mac_val$Cluster, ignore.case = T))
...
namer <- "FIGURES/FIGURE4/Figure4fg_martin_pops.pdf"
...
namer <- "FIGURES/FIGURE4/Figure4fg_martin_pops.png"
...
namer <- "FIGURES/FIGURE4/Figure4fg_martin_pops_simp.pdf"
...
####
namer <- "FIGURES/SF6/FigureSF6_martin_othr_pops.pdf"
pdf(namer, height = 5, width = 11)
...
namer <- "FIGURES/SF6/FigureSF6_martin_othr_pops.png"
png(namer, height = 5, width = 11, units = "in", res=200)
grid.arrange(ggSOX, bbSOX,
             ggAPOE, bbAPOE,
             nrow=2, widths = c(2,1.2))
...
write.table(ressy, "FIGURES/FIGURE4/Figure4_facs_stats_martins.txt", sep='\t', quote=F)